...
trait TextProcessingTrait {
   ...
   *
   * @param string $text
   *   The text to create a summary from.
   * @param int $size
   *   The size.
   *
   * @return bool|string
   */
  public function createSummary(string $text, int $size = 300) {
    ...
    // slice.
    $summary = CoreUnicode::truncate($text, $size);
  ...
  public function createHtmlSummary($html = '', $length = 100, $ending = '...') {
    ...
    while (!$finished) {
      ...
      elseif (preg_match('/^<\/(\w+)>/', $html, $matches)) {
        ...
        if (preg_match('/^([^<]+)(<\/?(\w+)[^>]*>)?/', $html, $matches)) {
          ...
          // utf and html entities/multi-byte characters to get accureate string
          // length.
    ...
    // If none and $return ends in an opening tag: (we ignore $finalSegment)
    if (strpos($finalSegment, ' ') === FALSE && preg_match('/<(\w+)[^>]*>$/', $return)) {
      ...
      // Otherwise, truncate $finalSegment to last space and add to $return:
    }
    else {
   ...
   */
  protected static function filterXssSplit(string $string, array $htmlTags, string $class) {
   ...
   */
  protected static function filterXssAttributes(string $attributes) {
    $attributesArray = [];
    $mode = 0;
    $attributeName = '';
    $skip = FALSE;
